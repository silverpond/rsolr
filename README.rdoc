...
When the Solr :wt is :json, then the response will be a Hash. This Hash is the same object returned by Solr, but evaluated as json. If the :wt is not :json, then the response will be a String.
...
== Response Formats
The default response format is Json. When the :wt param is set to :json, the response is parsed resulting in a Hash. All other response formats are available as expected, :wt=>'xml' etc..
